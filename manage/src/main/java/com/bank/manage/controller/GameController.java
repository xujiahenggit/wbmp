...
public class GameController {
    ...
    public Map<String, Object> queryGameQr(@RequestParam("macaddress") String macaddress, @RequestParam("type") String type, @RequestParam("timestamp") String timestamp, @RequestParam("sign") String sign, HttpServletRequest request) {
        ...
        if (StringUtils.equals(sin, sign)) {
            ...
            String localImgPath = configFileReader.getUPLOAD_FILE_PATH() + configFileReader.getQrImages() + "/" + DateUtil.today() + "/" + fileName;
            log.info("当前本地二维码图片存放地址：{}", localImgPath);
            FileUtil.mkParentDirs(localImgPath);
            String baseUrl = configFileReader.getTomcatBaseIp();
            String content = baseUrl;
            if (StringUtils.equals(type, "1")) {
                content += configFileReader.getHTTP_XQDZ() + macaddress;
            } else {
                content += configFileReader.getHTTP_DDL() + macaddress;
            }
            try {
    ...
    public DeviceDTO queryOrgcode(@RequestParam("macaddress") String macaddress, @RequestParam("timestamp") String timestamp, @RequestParam("sign") String sign) {
        ...
        if (StringUtils.equals(sin, sign)) {
            ...
            throw new BizException("设备入网校验，签名失败");
        }
        return deviceDTO;
    }
    ...
    public Object saveGame(@RequestParam("type") String type, @RequestParam("timestamp") String timestamp,
                           @RequestParam("sign") String sign, @RequestParam("gameresult") String gameresult) {
        ...
        try {
            ...
            if (sin.equals(sign)) {
                if (StringUtils.isNotBlank(gameresult)) {
                    ...
                    //返回这几个人的排名信息
                    List<GameDO> ranks = gameService.getGameRankInIds(type, ids);
                    data.put("ranking", ranks);
                }
                //前六名的信息
                data.put("resultlist", gameService.getGameRankByType(type, 6));
            } else {
    ...
    public GameDO queryRankById(@RequestParam("type") String type, @RequestParam("usernum") String usernum) {
        //返回这几个人的排名信息
        List<GameDO> ranks = gameService.getGameRankInIds(type, usernum);
        if (ranks.size()<1) {
    ...
    public boolean changeGame(@PathVariable String code, @PathVariable String deviceNo, String orgId) {
        ...
        String jmsPath = configFileReader.getMESSAGE_PATH();
