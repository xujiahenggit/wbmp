...
public class OperaCurveServiceImpl  implements OperateCurveService {
    ...
    public  float calcOrgMonthScore(String orgId, String date) {
       ...
       //全行普通客户标准客户量参数
        ...
        //获取数据库中的不同类型的客户量
        float normalCust =   Float.parseFloat(wbmpOperateCustDao.getOrgCustTypeNum(orgId, WbmpConstFile.CUSTOMER_TYPE_CUST_001,date));
        float goldCust =   Float.parseFloat(wbmpOperateCustDao.getOrgCustTypeNum(orgId, WbmpConstFile.CUSTOMER_TYPE_CUST_002,date));
        float platinumCust =   Float.parseFloat(wbmpOperateCustDao.getOrgCustTypeNum(orgId, WbmpConstFile.CUSTOMER_TYPE_CUST_003,date));
        float diamonCust =   Float.parseFloat(wbmpOperateCustDao.getOrgCustTypeNum(orgId, WbmpConstFile.CUSTOMER_TYPE_CUST_004,date));
        //计算分数
